...
def get_pie(site):
    ...
    if site == 'ALL Sites':
        ...
        pie_fig = px.pie(df, values='counter', names='class', 
                title='Site:'+site)
    return pie_fig   
    #return [dcc.Graph(figure=pie_fig)]
        
# TASK 4:
# Add a callback function for `site-dropdown` and 
# `payload-slider` as inputs, `success-payload-scatter-chart` as output
@app.callback \
(  Output(component_id='success-payload-scatter-chart', component_property='figure'),
     [Input(component_id='site-dropdown', component_property='value'),
        Input(component_id="payload-slider", component_property="value")]
)
def get_scatter(site,plslider):
    print(site,plslider)
    if site == 'ALL Sites':
        # scatter plot
        scatter_fig=px.scatter(spacex_df, y="class", x="Payload Mass (kg)", 
        ...
        df = spacex_df[spacex_df['Launch Site']==site]\
            ...
            'class']].reset_index()
        #print(df.head())
        df = df[df['Payload Mass (kg)'] >= plslider[0]]
        df = df[df['Payload Mass (kg)'] <= plslider[1]]

        scatter_fig=px.scatter(df, y="class", x="Payload Mass (kg)", 
            color="Booster Version Category",
            labels={"Booster Version Category":"Booster Version Category ("+site+")"})
        scatter_fig.update_xaxes(tickvals=list(range(0, 10001, 1000)))
    return scatter_fig 
